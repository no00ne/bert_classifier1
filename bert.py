...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # 应用掩码，注意这里的attention_mask包含-10000（一个大负数）来掩盖填充部分
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # 再次残差连接和层归一化
    layer_output = self.add_norm(attention_output, intermediate_output, self.out_dense, self.out_dropout,
                                 self.out_layer_norm)
